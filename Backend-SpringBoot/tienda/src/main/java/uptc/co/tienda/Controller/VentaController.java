...
// Registro producto en el carrito
    ...
    public VentaDTO saveProductoCompra(@RequestBody VentaEntity ventaEntity){
		...
		if(productoExistente.getCantidad()>=ventaEntity.getCantidad()){
			...
			else if(fact.getEstado()!=null ){
				fact = new FacturaEntity(fact.getUsuario());
				fact=fsi.saveFactura(fact); 
			}
			if(fact.getListaVentaEntity()!=null){
				for(VentaEntity venta: fact.getListaVentaEntity()){
					if(ventaEntity.getProducto().getCodigo()==venta.getProducto().getCodigo()){
						return new VentaDTO("El producto ya esta agregado al carro");
					}
			    }   
			}
